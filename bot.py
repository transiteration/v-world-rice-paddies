...
def convert_tif_to_png(compression_level=6):
    ...
    print(f"Converting {len(tif_files)} .tifs to .pngs and compressing them. Please wait a moment...")    
...
async def send_next_image_and_mask(update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id):
    ...
    while files:
        ...
        if os.path.exists(image_path) and os.path.exists(mask_path) and filename not in processing_files:
            ...
            keyboard = [
                [InlineKeyboardButton("Keep", callback_data=f"keep:{filename}"),
                 ...
                 InlineKeyboardButton("Confusing", callback_data=f"later:{filename}")]
            ]
            ...
            try:
                ...
                await context.bot.send_message(chat_id=chat_id, text=f"Failed to process image or mask: {e}")
        else:
            context.user_data["files"] = files
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if filename not in processing_files or processing_files[filename] != query.message.chat_id:
        await query.edit_message_text(text="This image is already processed or not assigned to you.")
        return

    try:
        if action == "move":
            shutil.move(os.path.join(mask_dir, filename), os.path.join(unclear_dir, filename))
            await query.edit_message_text(text=f"Removing: {filename}")
        elif action == "keep":
            shutil.move(os.path.join(mask_dir, filename), os.path.join(clean_dir, filename))
            await query.edit_message_text(text=f"Keeping: {filename}")
        elif action== "later":
            shutil.move(os.path.join(mask_dir, filename), os.path.join(later_dir, filename))
        ...
        await context.bot.send_message(query.message.chat_id, text=f"Failed to move or copy image: {e}")
